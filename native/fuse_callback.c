...
static int javafs_statvfs(const char *path, struct statvfs *fst)
{
   ...
   while (1)
   {
      ...
      fst->f_bsize   = (*env)->GetLongField(env, jStatfs, FuseStatfs->field.blockSize);
      fst->f_blocks  = (*env)->GetLongField(env, jStatfs, FuseStatfs->field.blocks);
      fst->f_bfree   = (*env)->GetLongField(env, jStatfs, FuseStatfs->field.blocksFree);
      fst->f_bavail  = (*env)->GetLongField(env, jStatfs, FuseStatfs->field.blocksAvail);
      fst->f_files   = (*env)->GetLongField(env, jStatfs, FuseStatfs->field.files);
      fst->f_ffree   = (*env)->GetLongField(env, jStatfs, FuseStatfs->field.filesFree);
      fst->f_namemax = (*env)->GetLongField(env, jStatfs, FuseStatfs->field.namelen);
      break;
   }
